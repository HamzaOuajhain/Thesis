...
class BlenderDataset(BaseDataset):
    def __init__(self, cfg, basedir, trainskip=1, 
                 downsample_factor=1, translation=0.0, 
                 sc_factor=1., crop=0):
        super(BlenderDataset, self).__init__(cfg)
    ...
    def load_poses(self, path):
        ...
        # Read coordinate convention from config
        convention = self.config['loading_config']['convention']
        ...
        if convention == 'Opencv':
            ...
            custom_config = self.config['loading_config'].get('custom', {})
            for i in range(len(self.img_files)):
                line = lines[i]
                c2w = np.array(list(map(float, line.split()))).reshape(4, 4)
                if custom_config.get('flip_x', False):
                    c2w[:3, 0] *= -1
                if custom_config.get('flip_y', False):
                    c2w[:3, 1] *= -1
                if custom_config.get('flip_z', False):
                    c2w[:3, 2] *= -1
                if 'rotation' in custom_config:
                    rotation = np.array(custom_config['rotation']).reshape(3, 3)
