...
class KeyFrameDatabase(object):
    def __init__(self, config, H, W, num_kf, num_rays_to_save, device) -> None:
        ...
        self.H = H
        self.W = W
        self.gradient_method = config['mapping'].get('gradient_method', 'both')

        # Validation of the gradient method
        if self.gradient_method not in ['rgb', 'depth', 'both']:
            raise ValueError("Invalid gradient method in config. Choose 'rgb', 'depth', 'both'.")
    ...
    def add_keyframe(self, batch, filter_depth=False):
        '''
        Add keyframe rays to the keyframe database using gradient-based sampling
        '''
        rgb_image = batch['rgb'].squeeze(0).cpu().numpy()
        depth_image = batch['depth'].squeeze(0).cpu().numpy()
        direction = batch['direction'].squeeze(0)

        # Compute gradients and select samples
        samples = self.select_samples_gradient(rgb_image, depth_image, self.num_rays_to_save)

        # Extract rays data for sampled pixels
        rays = self.get_rays_from_samples(direction, rgb_image, depth_image, samples)
    ...
    def select_samples_gradient(self, rgb_image, depth_image, num_samples):
        if self.gradient_method == 'rgb':
            gradient = self.compute_gradient_magnitude(rgb_image)
        elif self.gradient_method == 'depth':
            gradient = self.compute_gradient_magnitude(depth_image)
        elif self.gradient_method == 'both':
            rgb_gradient = self.compute_gradient_magnitude(rgb_image)
            depth_gradient = self.compute_gradient_magnitude(depth_image)
            gradient = rgb_gradient + depth_gradient

        # Normalize gradient
        normalized_gradient = (gradient - gradient.min()) / (gradient.max() - gradient.min())

        # Flatten the gradient magnitude
        flat_gradients = normalized_gradient.flatten()
    ...
    def get_rays_from_samples(self, direction, rgb_image, depth_image, samples):
        rays = torch.cat([
            direction[samples[:, 0], samples[:, 1]],
            torch.from_numpy(rgb_image[samples[:, 0], samples[:, 1]]),
            torch.from_numpy(depth_image[samples[:, 0], samples[:, 1], None])
        ], dim=-1)

        return rays.unsqueeze(0)  # Add batch dimension to match original shape
